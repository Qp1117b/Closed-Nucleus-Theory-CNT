...
规范耦合常数的归一化:
- 在ħ = c = 1, ε₀ = 1 的理想化约定下，α = e²/(4π)
- 在SI单位制中：α = e²/(4π ε₀ ħ c)
- 4π因子来自球面立体角的积分
-/
...
theorem four_pi_factor_origin :
  -- 电磁作用量（SI单位）:
  -- S = -1/(4) ∫ F_μν F^μν d⁴x
  -- 其中场强张量 F_μν 的量纲为 [M¹/² L⁻¹/² T⁻¹]，由 √(ε₀) 归一化
  -- 4π因子来自球面立体角的积分
  -- ∮_{S²} dΩ = 4π
  True := by
  trivial
...
本文件推导了精细结构常数 α 的纯数学表达式。

CNT 框架仅以 ℏ 和 c 为输入。α 不是外部输入参数，
它是 4-单纯形几何的代数结果。

理论值: 1/α₀ = 16384π/375

此推导仅涉及几何量（π、整数355、375、中点正弦），
不依赖任何外部物理常数。
-/
