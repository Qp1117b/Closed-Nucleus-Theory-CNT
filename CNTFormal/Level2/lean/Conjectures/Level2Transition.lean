...
本文件形式化闭合核理论（CNT）的二级质变的完整理论体系。
在一级质变建立穿刺网络后，当Logistic饱和达到拐点N=N_max/2时：
...
依赖关系：
  ...
  - CNTConjectures.Level1Transition（一级质变：穿刺网络）
...
/- ======================================================================
  ...
  一级质变中，N: TotalParticles = "改造粒子数"。在穿刺网络形成后
  （二级质变的起点），再生产改造的不再只是粒子，还有网络穿刺本身。

  重记方案：
    N̂_k = (N_A(k), N_B(k), P_A(k), P_B(k))

  其中：
    N_A, N_B = A/B网络的粒子数（继承一级质变）
    P_A, P_B = A/B网络的穿刺数（新增——二级质变专属）

  穿刺的"标记"含义：
  - 一级质变：标记是对径面（历史面）的累积，触发质变
  - 二级质变：标记是穿刺网络连接状态，决定交换耦合强度
...
/-- 二级质变再生产记录：粒子数 + 网络穿刺数
    ...
    与一级质变的区别：
    - 一级质变只标记"粒子"（历史面的再生产次数）
    - 二级质变同时标记"粒子"和"网络穿刺"（穿刺的连接强度）
...
/- ======================================================================
  ...
  J₀ = √(3/5) / 10 来自4-单纯形几何的完整推导链：

  γ = 1/√15 (Barbero-Immirzi参数，来自二面角cosΘ=1/4)
  Γ = 3/5  (几何因子 = 可见面数/总顶点数)
  J = γ·Γ = 3/(5√15) = √15/25 (总耦合)
  J₀ = J/2 = √15/50 = √(3/5)/10 (单通道交换耦合)
...
/- ======================================================================
  ...
  级质变中的单纯粒子计数。每个再生产步同时更新粒子数和穿刺数。
  ======================================================================-/

/- 一级质变后的网络再生产方程（原始版本，见 Level1Transition.lean）
  ...
  eq_A_N: A网络粒子数演化
  eq_B_N: B网络粒子数演化
  eq_A_P: A网络穿刺数演化（新增——二级质变特有）
  eq_B_P: B网络穿刺数演化（新增——二级质变特有） -/
...
/- ======================================================================
  ...
  §6.2 光子的性质
    ...
    自旋：s = 1（矢量玻色子）
    规范耦合：与电荷 q 成正比
    传播速度：c = √2 · ℓ₀ · f_c（继��一级质变的光速，量纲 [L·T⁻¹]）
...
/- ======================================================================
  ...
  光子传播速度 c 不是二级质变独立设定的，而是从一级质变继承：
    c = √2 · ℓ₀ · f_c
...
/- ======================================================================
  ...
  一级质变 (L1)          中间态 (网络化)        二级质变 (L2)
  ─────────              ────────────           ─────────
  N_c = 3                穿刺网络扩展            N_c = 34 (Logistic拐点)
  f_c = 1                ℤ₂ 交换对称性           ℤ₂ → U(1) 提升
  c = √2                 交换耦合 J₀             α = ||C||²/(π·34)
  l_min = √2             │                      电荷量子化
  j = 1/2                │                      光子涌现
  │                       │                      │
...
/- ======================================================================
  ...
  一次质变后，闭合核的尺度长度 ℓ₀ 来自三个基本结构：
    1. 一级质变的几何关系：c = √2 · ℓ₀ · f
    2. 一次再生产的能量子：ε = Nn · ħ · f
    3. 临界条件 f_c = 1（一次质变的最小频率）
