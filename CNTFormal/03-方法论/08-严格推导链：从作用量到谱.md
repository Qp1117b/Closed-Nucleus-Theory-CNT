...
卡当矩阵谱 $\lambda_k = 2 - 2\cos(k\pi/5)$。通过 Friedrichs 扩张的边界参数与迹 $\text{Tr}(A^{-1}) = 2$，连续化后有效边界势：
...
由卡当矩阵特征值通过 Adele-谱对应严格确定（映射已确认）。
...
| 谱间隙 $c^2/4$ | ✅ Sturm-Liouville | 卡当矩阵 Friedrichs 扩张涌入 |
