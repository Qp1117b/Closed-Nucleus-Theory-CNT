...
rho_2 = mp.mpf('0.19907')
rho_3 = mp.mpf('0.10197')
...
print('    C_th = C/E_1                  → 精确')
print('    N_m (SU(5) Weyl归一化)       → 群论确定（N_2^2=8/15, N_3^2=8/9）')
print()
print('  CNT Cartan-S5 adelic 周期预测:')
print(f'    1/alpha_EM ≈ {1/float(alpha_inv_base):.3f}')
print(f'    偏差: {(float(alpha_inv_base)-float(target))/float(target)*1e6:.0f} ppm')
print()
print('  rho_m 角向修正（SU(5) Mathieu重叠积分）:')
print('    rho_2 = |<psi_2|sin(2θ)|psi_1>|^2  ≈ 0.222')
print('    rho_3 = |<psi_3|cos(4θ)|psi_1>|^2  ≈ 0.077')
print()
print('  当前可达到的最优精度 (不改非第一性输入):')
print(f'    alpha^{-1}: {float(alpha_inv_base):.6f} → 与实验差 {diff*1e6/float(target):+.0f} ppm')
