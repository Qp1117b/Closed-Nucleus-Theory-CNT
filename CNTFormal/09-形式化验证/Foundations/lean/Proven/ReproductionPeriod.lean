...
**层级区分**：
  ...
  ★ 一级量变质变（网络化）：多个闭合核通过再生产连接，再生产频率 f 涌现
...
/- ======================================================================
  ...
  ★ 再生产频率 f 是网络化后涌现的概念 ★
    一级量变质变后，ν 在网络行为层面表现为再生产频率 f_rep = ν。
...
/-- 再生产频率 f（网络化后涌现）
    一级量变质变后，能量子频率 ν 表现为再生产频率 f_rep = ν。
...
/- ======================================================================
  ...
  ★ 重要澄清 ★
  辐射速度 v_rad 不是前网络概念，而是网络化（一级量变质变）后涌现的物理量。
  ...
  辐射速度的严格定义和证明见：
    - 质量涌现 → QCD能动张量-朗兰兹-几何构型（参见 CNT_研究展开链.md）
...
/- ======================================================================
  ...
  4. 再生产频率 f_rep 与再生产周期 T_rep
     ...
     - 一级量变质变后，ν 转化为再生产频率 f_rep = ν
     - T_rep = 1/f_rep 是再生产周期

  5. 辐射速度是网络化涌现量（非前网络概念）
     - 前网络阶段：d·ν 无界（形式距离 d 无界）
     - 网络化后：c = √2·ℓ₀·f_rep 有界（信息传播限制在4-单纯形内）
     - 严格定义和证明：质量涌现 → QCD能动张量-朗兰兹-几何构型（参见 CNT_研究展开链.md），以及 RepRadioSpeed.lean
  ...
  ★ 因果链修正 ★
    ...
    一级量变质变：f_rep = ν（再生产频率涌现）+ c = √2·ℓ₀·f_rep（涌现速度）
