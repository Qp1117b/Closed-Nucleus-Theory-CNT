...
/-- 再生产周期类型：正实数时间
    与主库 CNTFormal.ReproductionPeriodStrict 是相同底层结构，
    独立定义以避免跨命名空间的 Coe 实例冲突 -/
...
/-- 量产物的能量形式标记（自然数，越大表示越复杂的内部结构）
    注意：与主库 CNTFormal.KernelPerspective.FormNumber（ℕ×ℕ×ℕ）是不同概念 -/
abbrev EnergyFormNumber := ℕ
...
structure MaterialFormConservation where
  ...
  /-- 再生产前的形式标记 -/
  input_form : EnergyFormNumber
  /-- 再生产后的形式标记 -/
  output_form : EnergyFormNumber
  /-- 形式变化（第一条）：再生产改变了形式 -/
  form_transformed : output_form ≠ input_form
  /-- 再生产周期 -/
  tau : ReproductionPeriod
  /-- n-τ关系：一个再生产周期内完成n个能量子的形式改造 -/
  irreversible : ¬ ∃ (pred : EnergyFormNumber → EnergyFormNumber), pred output_form = input_form
...
-/
structure ReproductionSignature (C : Type) [Category C] [Foundations.Strict.CNTCategory C] (S : C) where
  /-- 再生产周期 -/
  tau : ReproductionPeriod
  /-- 每个周期的能量子消耗 -/
...
合起来：再生产使形式以单调方向演化，
        ...
        能量子基底保守不变。
-/
def fitnessRelatesToForm (_fitness : ℝ) (_form : EnergyFormNumber) : Prop := True
...
形式距离：绝对值差异作为自然度量
-/
noncomputable def formDist (f1 f2 : EnergyFormNumber) : ℝ := |(f1 : ℝ) - (f2 : ℝ)|

theorem formDist_nonneg (f1 f2 : EnergyFormNumber) : formDist f1 f2 ≥ 0 :=
  abs_nonneg _

theorem formDist_symm (f1 f2 : EnergyFormNumber) : formDist f1 f2 = formDist f2 f1 := by
  dsimp [formDist]
  rw [abs_sub_comm]

theorem formDist_self (f : EnergyFormNumber) : formDist f f = 0 := by
  dsimp [formDist]
  simp

theorem formDist_triangle (f1 f2 f3 : EnergyFormNumber) : formDist f1 f3 ≤ formDist f1 f2 + formDist f2 f3 := by
  dsimp [formDist]
...
theorem formDist_zero_iff (f1 f2 : EnergyFormNumber) : formDist f1 f2 = 0 ↔ f1 = f2 := by
  dsimp [formDist]
  constructor
  · intro h
...
structure FormMark where
  f_nuc : EnergyFormNumber
  f_raw : EnergyFormNumber
  f_marked : EnergyFormNumber
...
-/
def inCausalCone (cone : CausalCone) (f : EnergyFormNumber) : Prop :=
...
/-
辐射速度上限的主推导已移至主库 CNTFormal.SimplexDominance.lean：
  radiative_velocity_bounded 定理从4-单纯形紧致性严格推导了
  v_rad ≤ diam(Δ₄)/τ，不依赖于额外 axiom。

本处的 radiative_velocity_upper_bound 已废弃，由主库几何推导取代。
-/
