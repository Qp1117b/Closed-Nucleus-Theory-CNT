...
> **摘要**
>
> 本文针对闭合核理论（CNT）框架进行两项核心修正：(1) 基于 Lindquist (1966)、Ehlers (1971)、Israel (1972) 和 Sarbach & Zannias (2013) 的相对论动理学标准框架，给出方程③（测地线通量密度）的严格良性定义，消除原方程的量纲不匹配、协变性缺失、边界/体混淆及初始条件不清等缺陷；(2) 将引力机制从"熵力中介"替换为"曲率直接响应"——直接采用 Einstein 场方程描述引力=曲率，能动张量量子化为离散能量子贡献的求和，引力常数 $G$ 作为连接量子化能动张量与时空曲率的耦合常数（汇率）。修正后的框架分为**质量涌现方程组**（方程①–⑥）和**曲率直接响应方程组**（方程⑦–⑨），通过质量涌现（方程⑤）自然衔接，构成从几何到质量到能动张量到曲率的完整因果闭环。

**关键词**：闭合核理论；测地线通量；相对论动理学；质量涌现；曲率直接响应；量子化能动张量；耦合常数；全息原理
...
本文给出两项修正后的完整九方程体系。修正二直接采用 Einstein 场方程，将引力视为时空曲率的直接响应：能动张量由量子化能量子贡献求和（方程⑧），引力常数 $G$ 作为连接量子化能动张量与曲率的耦合常数/汇率（方程⑨），无需熵力中介。
...
> **【架构声明】** E7-E9（Einstein场方程/量子化能动张量/G作为耦合常数）直接采用广义相对论的标准框架，将引力视为时空曲率的直接响应。E7-E9与E1-E6通过方程⑤（质量涌现）衔接：质量是能动张量的源，能动张量决定曲率，曲率反过来通过测地线方程①约束能量子运动，形成闭环。$G$ 作为耦合常数由闭合核网络几何结构决定（见方程⑨），不再是自由参数。本组方程替代了原"信息场方案"（方案B）与"纯测地线方案"（方案A'）——二者均通过熵力/熵-压间接中介引力，现统一为曲率直接响应。

### 方程⑦：Einstein场方程（引力=曲率）

$$\boxed{G_{\mu\nu} = \frac{8\pi G}{c^4} T_{\mu\nu}}$$

引力即时空曲率，由 Einstein 场方程直接描述。Einstein张量 $G_{\mu\nu}$（曲率）由能动张量 $T_{\mu\nu}$ 直接决定。无需熵力中介，无需测地线汇聚→碰撞率→熵压的间接链条。
...
能动张量是量子化的。每个被闭合核束缚的能量子 $h\nu_i$ 贡献一份离散的能动张量 $T_{\mu\nu}^{(i)}$。能量子是一份份的，能动张量也是一份份的。质量（由方程⑤涌现：$m = \frac{h\nu}{c^2}\int_S \rho \sqrt{\gamma} d^2x$）是 $T_{\mu\nu}$ 的源。
...
**物理诠释**：在CNT中，能量子是基本离散单元。当能量子被闭合核束缚时，产生质量（方程⑤）。质量是能动张量的源。因此 $T_{\mu\nu}$ 本质上是量子化的——它是离散能量子贡献的求和。

### 方程⑨：G作为耦合常数（汇率）

$$\boxed{G = \frac{c^4}{8\pi} \cdot \frac{G_{\mu\nu}}{T_{\mu\nu}}}$$

$G$ 是连接量子化能动张量 $T_{\mu\nu}$ 和时空曲率 $G_{\mu\nu}$ 的"汇率"——一个比例常数。其数值由闭合核网络的几何结构决定。在CNT框架中，$G$ 不是自由参数，而是从4-单纯形网络几何涌现的耦合常数。
...
| ⑦ | $G_{\mu\nu} = \frac{8\pi G}{c^4} T_{\mu\nu}$ | Einstein场方程（引力=曲率） |
| ⑧ | $T_{\mu\nu} = \sum_i h\nu_i \, u_\mu^{(i)} u_\nu^{(i)}$ | 量子化能动张量 |
| ⑨ | $G = \frac{c^4}{8\pi} \cdot \frac{G_{\mu\nu}}{T_{\mu\nu}}$ | G作为耦合常数（汇率） |

### 5.2 引力=曲率直接响应

引力不再通过熵力/熵-压间接涌现，而是由 Einstein 场方程直接描述为时空曲率。因果链条为：

$$\text{几何（方程①–⑥）} \xrightarrow{m} \text{方程⑧} \xrightarrow{T_{\mu\nu}} \text{方程⑦} \xrightarrow{G_{\mu\nu}} \text{方程⑨} \xrightarrow{G}$$
...
$$\text{方程①} \to \text{方程⑤} \xrightarrow{m} \text{方程⑧} \xrightarrow{T_{\mu\nu}} \text{方程⑦} \xrightarrow{G_{\mu\nu}} \text{方程⑨} \xrightarrow{G}$$
...
| 引力机制 | 方案B（信息场）/方案A'（纯测地线）：几何→测地线汇聚→碰撞率→熵-压响应→引力 | 曲率直接响应：几何→质量→$T_{\mu\nu}$→$G_{\mu\nu}$（Einstein场方程直接描述） |
| 方程⑦ | 测地线汇聚因子 $K_M$（方案A'）/ 信息梯度场 $\mathcal{G}_M$（方案B） | Einstein场方程 $G_{\mu\nu} = \frac{8\pi G}{c^4} T_{\mu\nu}$（引力=曲率） |
| 方程⑧ | 边界碰撞率 $J(\theta)$（方案A'）/ 玻尔兹曼偏置 $n(\theta)$（方案B） | 量子化能动张量 $T_{\mu\nu} = \sum_i h\nu_i u_\mu^{(i)} u_\nu^{(i)}$ |
| 方程⑨ | 熵-压响应 $P(\theta)$（方案A'） | $G$ 作为耦合常数（汇率）$G = \frac{c^4}{8\pi} \cdot \frac{G_{\mu\nu}}{T_{\mu\nu}}$ |

### 开放参数

| 参数 | 位置 | 解决方向 |
|:---|:---|:---|
| $G$ 的几何起源 | 方程⑨ | 从4-单纯形网络几何与 $\ell_0/\ell_P = \sqrt{\pi\sqrt{2}/\varepsilon_2}$ 严格推导 |
| $\hat{f}_0$ | 方程③ | 从量子引力或全息原理推导 |
| $T_{\mu\nu}^{(i)}$ 的严格形式 | 方程⑧ | 能量子四维速度 $u_\mu^{(i)}$ 的微观统计力学模型 |
